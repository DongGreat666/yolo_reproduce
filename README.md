...
| difficult 目标 | VOC 官方评估忽略 difficult | 未忽略 |
| 论文结果 | VOC07 mAP 63.4% | 当前最好约49.4% |
| 速度 | 45 FPS | 未做 |
...
| 最佳结果 | VOC07 mAP 63.4% | 约6% | 约32.1% | 约42% |

1. 在验证了实验参数的有效性后，继续复现YOLOv1，以此与实验一进行对照，证明预训练的重要性。首先使用 VOC2007+VOC2012 对 Darknet 做多标签分类预训练，再加载主干权重训练 YOLOv1 检测器。分类预训练最高 Cls mAP 约为0.50；最终检测结果best mAP50 约为 0.321。
...
| 最佳结果 | VOC07 mAP 63.4% | 约6% | 约32.1% | 约34.7% | 约42% | 约49.4% |
...
（2）实验结果表明，使用 CNN 检测头的模型在收敛速度和最终性能上均优于 FC 检测头。Darknet VOC 预训练 + CNN 检测头的 best mAP50 约为 0.347，相比 Darknet VOC 预训练 + FC 检测头的 best mAP50 约为 0.321，提升约 0.026，即 2.6 个百分点。ResNet34 ImageNet 预训练 + CNN 检测头的 best mAP50 约为 0.494，相比 ResNet34 ImageNet 预训练 + FC 检测头的 best mAP50 约为 0.420，提升约 0.074，即 7.4 个百分点。
...
| mAP50 | **0.634** | 0.060 | 0.321 | 0.347 | 0.420 | **0.494** |
...
由于实验中预训练Backbone使用的 VOC 数据集较小，且训练轮数有限；而且在计算mAP50未排除 **difficult 目标** 的影响；另外在尽可能地保证实验参数一致的情况下，个别细节有所不同，最终 mAP50 仍低于原论文的 0.634。未来可以尝试更大规模的数据集（如 COCO）以及更优的网络结构，以进一步提升性能。

##### 2.3.2 示例图——0.25的置信度阈值
